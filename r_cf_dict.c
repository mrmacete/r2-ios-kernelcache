...
static void r_cf_value_print(RCFValue * value);

#ifndef R_CF_DICT_IN_LIB
int main(int argc, char * argv[]) {
	if (argc < 2) {
		eprintf ("USAGE: cfdict-parse dict.xml\n");
		return 0;
	}

	RBuffer * file_buf = r_buf_new_slurp (argv[1]);
	if (!file_buf) {
		eprintf ("Error loading file\n");
		return 1;
	}

	ut64 size = r_buf_size (file_buf);
	RCFValueDict * dict = r_cf_value_dict_parse (file_buf, 0, size, R_CF_OPTION_SKIP_NSDATA);
	r_buf_free (file_buf);

	if (dict) {
	  r_cf_value_dict_print (dict);
...
RCFValueDict * r_cf_value_dict_parse(RBuffer * file_buf, ut64 offset, ut64 size, int options) {
  ...
  xml_buf = malloc(XMLBUFSIZE);
  yxml_init(&x, xml_buf, XMLBUFSIZE);

  stack = r_list_newf ((RListFree)&r_cf_parse_state_free);
  if (!stack) {
	  goto beach;
  }
